...
async function loadCitizenHome() {
    ...
    try {
        const list = await getJSON(`${API}/complaints/user/${uid}`);
...
async function loadOfficerHome() {
    ...
    if (!dept) return;
    try {
        const s = await getJSON(`${API}/complaints/stats/department/${dept}`);
...
async function loadAdminHome() {
    ...
    try {
        const s = await getJSON(`${API}/complaints/stats`);
...
async function fileComplaint(ev) {
    ...
    try {
        const r = await postJSON(`${API}/complaints`, { title, description: desc, priority, category, userId: parseInt(uid), performedBy: name });
        if (r.ok) {
...
async function loadRoleComplaints() {
    ...
    try {
        ...
        if (role === 'OFFICER') {
            if (dept) {
                list = await getJSON(`${API}/complaints/department/${dept}`);
...
async function updateStatus(id) {
    ...
    try {
        const r = await putJSON(`${API}/complaints/${id}/status`, { status, performedBy: name });
        if (r.ok) {
...
async function saveEdit() {
    ...
    try {
        const r = await putJSON(`${API}/complaints/${editingId}`, { title, description, priority, performedBy: name });
        if (r.ok) {
...
async function deleteComplaint(id) {
    ...
    try {
        const resp = await fetch(`${API}/complaints/${id}`, { method: 'DELETE' });
        if (resp.ok) {
...
async function assignComplaint(id) {
    ...
    try {
        const r = await putJSON(`${API}/complaints/${id}/assign`, { assignedTo: name.trim(), performedBy: by });
        if (r.ok) {
...
async function viewHistory(id) {
    ...
    try {
        const list = await getJSON(`${API}/complaints/${id}/history`);
...
async function loadProfile() {
    ...
    if (!uid) return;
    try {
        const d = await getJSON(`${API}/users/${uid}`);
